...
//---------------------------------------------------------------------------------------------------------------//
	...
	public boolean insert(Member member){
		 ...
		 String memberUrl = member.getMemberUrl();
		 String lastReviewDate = member.getLastReviewDate();
		 String phoneNumber = member.getPhoneNumber();
		 int point = member.getPoint();
		 int level = member.getLevel();
		 int reviewCount = member.getReviewCount();
		 int totalReivewCount = member.getTotalReivewCount();
			
				
		String sqlStatement="insert into membertb (id, password, name, memberUrl, lastReviewDate, phoneNumber, point, level, reviewCount, totalReivewCount) values (?,?,?,?,?,?,?,?,?,?)";
		return (jdbcTemplateObject.update(sqlStatement, new Object[]{id, password, name, memberUrl, lastReviewDate, phoneNumber, point, level, reviewCount, totalReivewCount})==1);
	}
	...
	public boolean update(Member member){
		...
		String sqlStatement="update membertb set id=?, password=?, name=?, memberUrl=?, lastReviewDate=?, phoneNumber=?, point=?, level=?, reviewCount=?, totalReivewCount=? where id=?";
		return (jdbcTemplateObject.update(sqlStatement, new Object[]{id, password, name, memberUrl, lastReviewDate, phoneNumber, point, level, reviewCount, totalReivewCount})==1);
	}
	
	public boolean delete (int id) {
		
		String sqlstatement="delete from membertb where id=?";
